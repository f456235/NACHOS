...
static void Copy(char *from, char *to)
{
    ...
    DEBUG(dbgFile, "main: Open: "<<to2);
    openFile = kernel->fileSystem->OpenAFile(to2); // orig: to
...
int main(int argc, char **argv)
{
    ...
    if (removeFileName != NULL)
    {
        ...
        /*li1_4*/
        kernel->fileSystem->Remove(removeFileName, recursiveRemoveFlag ? TRUE : FALSE);
    }
    ...
    if (dirListFlag)
    {
        ...
        /*li1_4*/
        kernel->fileSystem->List(listDirectoryName, recursiveListFlag ? TRUE : FALSE);
    }
    if (mkdirFlag)
    {
        // MP4 mod tag
